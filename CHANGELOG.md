...
- **The most severe duplicate finding was discarded for the first-seen one** (`runner._prioritize`). Findings were deduped by lowercased title *before* the severity sort, and dedupe keeps the first occurrence — so the survivor of a duplicated title was whichever scanner happened to sit earlier in `REGISTRY`, not the one reporting the worse problem. A `critical` reported by a later scanner was silently dropped in favour of a `low` with the same title, and the "Top recommendations" list — the one thing on the report that exists to rank by severity — understated it. Now sorts first and dedupes second, which is what the function's own docstring has always said; it was `CLAUDE.md` that documented the reverse, and that note is corrected too. Verified across 400 randomised no-collision cases that the two orders produce byte-identical output, so nothing changes for the scanners as they stand — no two of them currently emit the same title. It matters the moment one does.
- **The report heading's link broke on characters the URL validator accepts** (`report.py`). The markdown heading wraps the scanned URL in a code span and an angle-bracket link destination, and both broke on input `normalize_url` lets through: a literal `>` ended the destination early and leaked the remainder into the document as text, and a backtick closed the code span and took the entire link construct with it — `https://example.com/?q=%60x%60` rendered the heading with **no link at all**. Verified against a real markdown engine, not by inspection. Backslash escaping is not usable here (CommonMark gives code spans higher precedence than links, so a backtick in a destination is consumed before the link is parsed), so the destination percent-encodes the handful of genuinely unusable characters while leaving existing `%XX` escapes alone, and the code span now picks a backtick fence longer than any run inside the URL. Seven URL shapes covered, including parentheses, `_*~`, and pre-encoded escapes.
- **The parent walk climbed into the public suffix and read the registry's DNS** (`publicsuffix.py`, new; `caa.py`, `email_auth.py`, `registration.py`). Three separate copies of a "take the last two labels" rule decided where a hostname stops belonging to its owner. Under a multi-label public suffix that is one label too far: `example.co.za` resolved its apex to `co.za`. **Not hypothetical** — `co.za` publishes MX *and* SPF records and `com.mx` publishes SPF, both verified live over DoH, so a domain under either inherited email authentication it had never configured and scored for it on a **weight-2.0** scanner, while the registry's MX also suppressed the "not a mail-sending host" link-out that should have applied. The CAA walk would likewise credit a domain with a registry's issuance policy. RDAP was the benign case (`www.example.co.uk` asked about `co.uk`, got nothing, rendered no registration card). All three now share one boundary in a new `publicsuffix` module whose walk stops at the registrable domain. Verified end to end: `www.bbc.co.uk` now resolves RDAP to `bbc.co.uk` and renders a registration card that previously did not appear at all. The list is curated rather than the full PSL — a deliberate trade against a dependency whose bundled data would age inside our pin, and an unlisted suffix behaves exactly as everything did before, so incompleteness costs nothing against the status quo.
- **Platform subdomains inherited the platform's DNS** (`publicsuffix.py`). The curated suffix list covered registry suffixes (`co.uk`, `co.za`) but not the PSL "private section" — the platforms that hand out subdomains. Whoever owns `myapp.vercel.app` does not control `vercel.app`'s DNS, but the walk climbed there anyway. Verified live over DoH: `vercel.app` and `netlify.app` publish SPF **and** DMARC, `github.io` publishes SPF and six CAA records, `amazonaws.com` publishes MX, SPF and DMARC. A scan of `myapp.vercel.app` — a host that does not exist — returned **A+/100 on both CAA and email auth**, entirely on Vercel's records. This was the more damaging half of the suffix bug, because these hosts are far likelier to be scanned than a ccTLD second-level. Now scores D/40 and F/0, which is what a host with no records of its own has earned.
- **Three-label suffixes were unrepresentable, not merely absent** (`publicsuffix.py`). `public_suffix` compared only the last two labels, so an entry like `s3.amazonaws.com` could sit in the table and never be consulted: `bucket.s3.amazonaws.com` matched `amazonaws.com`, stopped there, and left the walk inside `s3.amazonaws.com` — itself a suffix. A gap that looks closed when you read the table is worse than an obvious one. Now longest-match.
- **Email auth graded a target that has no domain** (`email_auth.py`). SPF, DMARC and DKIM are published against a name someone owns; an IP literal or bare public suffix has none. Scanning `https://1.1.1.1` produced a fabricated **F/0** feeding the weighted average at weight 2.0. This was masked until the suffix fix landed: the old walk invented parent names (`1.1.1.1` → `1.1.1` → `1.1`) and tripped the subdomain link-out by accident, so tightening the walk exposed it. Now says "not applicable" on purpose and is excluded from the score.
- **CAA graded the same targets** (`caa.py`). `https://1.1.1.1` reported "No CAA records found" with a **D/40** — stating a finding about a name that cannot have one, and feeding that grade into the average. Pre-existing, unlike the email-auth case. Now a link-out.
- **CAA stopped climbing at the public suffix, and said so out loud** (`caa.py`, `publicsuffix.py`). A regression introduced by the previous fix in this same release: the public-suffix boundary was applied to CAA as well as to SPF/DMARC, but the three protocols do not climb the same distance. **SPF** (RFC 7208) does not climb at all; **DMARC** (RFC 7489 §6.6.3) falls back only to the Organizational Domain, defined *using a public suffix list*; **CAA** (RFC 8659 §3) walks `domain -> Parent(domain)` toward the root and does **not** exclude public suffixes. So `vercel.app`'s CAA genuinely constrains issuance for `myapp.vercel.app`, and the scanner was reporting "No CAA records on this domain or any ancestor" about a host whose issuance *is* restricted — false, and false in the reassuring direction. CAA now uses `ancestor_domains` while email auth keeps `parent_domains`, and when the match lands above the registrable domain the report adds an explicit finding that the policy is inherited from the platform or registry and is not the owner's to change.
- **HSTS preload graded a target that cannot have HSTS** (`hsts_preload.py`). RFC 6797 §8.1.1 says a user agent MUST NOT apply HSTS to a host identified by an IP address, and the preload list holds names only. `https://1.1.1.1/` nonetheless returned **B+/80 "Domain is NOT on the HSTS preload list"** — a number about a question that was never asked, feeding the weighted average. Now a link-out.
- **DNSSEC told an IP target to check its spelling** (`dnssec.py`). A DoH SOA query for `1.1.1.1` returns NXDOMAIN, which the scanner rendered as *"The domain does not exist. Check spelling and registration status"* — advice about a misspelled domain, for a target that is not a domain. It was `ok=False` so it never corrupted the score, unlike the two above, but the guidance was simply wrong. Now a link-out that says DNSSEC signs zones and an IP has none.
- **The public-suffix boundary now uses the real Public Suffix List** (`publicsuffix.py`, `publicsuffixlist>=1.0.2,<2`). It had been a hand-curated table — correct for what it listed and silently wrong for everything else. Measured against the real list, that table **invented three entries that are not suffixes at all** (`amazonaws.com`, `k12.us`, `netlify.live` — plausible, written in good faith, simply wrong) and **omitted every one of nine sampled ccTLD families** (`.ug`, `.tz`, `.gh`, `.rw`, `.fj`, `.np`, `.lk`, `.mm`, `.kh`), which fell back to last-two-labels and the original bug. The data is bundled, so a scan still makes no network call it did not already make, and the boundary's freshness now tracks a package upgrade rather than someone remembering to add a line.
- **The PSL's PRIVATE section is included deliberately**, and **IP literals are screened before the library sees them**. Restricting to the ICANN section would put the boundary for `myapp.vercel.app` at `vercel.app` — exactly the bug this module exists to prevent. And `PublicSuffixList().privatesuffix("1.1.1.1")` returns `"1.1"`: it reads an address as a domain, so delegating without the `is_ip_literal` guard would have re-opened the fabricated-grade bugs on IP targets. Both are pinned by tests. 35 of the 37 existing boundary tests passed unchanged across the swap, which is what made it safe to make; the two that failed were asserting the curated table's own mistakes.
- **IPv6 targets still got a fabricated grade — the same bug, half-fixed** (`publicsuffix.py`). `parent_domains` distinguished "bare public suffix" from "single-label host" by counting dots, and an IPv6 literal has none, so `2001:db8::1` was read as a single-label hostname and handed back as its own leaf. `email_auth` then walked it, found nothing, and scored **F/0 at weight 2.0** — while the identical IPv4 target was correctly excluded, because `caa` guards with `is_ip_literal` directly and `email_auth` relied on the heuristic. `normalize_url` accepts bracketed IPv6 and there is a test pinning that, so this was reachable. IP literals are now screened explicitly in every entry point.
...
- **RDAP is now cached per registrable domain** (`registration.py`). Registration data changes about once a year, so re-querying a registry on every scan was pure waste — and not harmless waste, since that is precisely what earns the HTTP 429 the entry above deals with. Successful lookups are cached for 24h, failures for 10 minutes. The short negative TTL is the point of the split: a negative entry is what stops a rate-limiting registry being hit once per scan, while 10 minutes stops a single transient blip hiding the registration card for a day. Measured: three scans of `www.bbc.co.uk`, `bbc.co.uk/path` and `news.bbc.co.uk` now make **one** RDAP call instead of three, and five scans against a 429-ing registry make **one** instead of five. The cache is bounded at 512 entries with prune-then-evict-oldest, because an unbounded dict keyed by a user-supplied domain is a slow memory leak on a public service — the same defect `urlutil._DNS_CACHE` carried until earlier in this release. Keyed on `time.monotonic()` so a clock step cannot resurrect or freeze an entry.
...
- **Coverage added for the two thinnest scanners**, separately from the fixes above — no defect had shipped in either. `mozilla_observatory` went from 1 test to 13 and `internetnl` from 1 to 6, taking the suite from 184 to **201**. Observatory now pins the upper score clamp (it awards bonus points, so >100 is real and would push the weighted mean past its own ceiling), the grade→score fallback (without it the scanner drops out of the average silently while still reporting `ok=True`), the severity thresholds that order "Top recommendations", four malformed `tests` payload shapes, and HTTP/non-JSON errors returning `ok=False` rather than a fabricated low grade. internetnl now asserts it makes **zero** HTTP calls rather than merely claiming so in a docstring, and that a configured token reaches neither the report nor the logs. Each new test was mutation-checked — the clamp, the fallback, the thresholds and the payload guard were each broken in turn to confirm exactly one test caught it.
- **`caplog` does not work for `urlreporter.*` loggers in this suite.** `logging_setup.setup_logger()` sets `propagate = False` on the package logger and `urlreporter.web` calls it at import time, so pytest's `caplog` — which attaches to the *root* logger — silently captures nothing once any test has imported the web module. A test written against it passes alone and fails in the full suite, on collection order alone. Attach a handler to the module's own logger instead.
...
- **Every fix carries a regression test verified to fail against the unfixed code** — the source file was reverted, the test run, the failure confirmed, then restored. The suite grew from 121 tests to 184 through the fixes; further coverage work since took it to 204.
